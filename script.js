...
// -------------------------------------------------------------------------
// FUNÇÕES PARA EXIBIR/OCULTAR DESCRIÇÃO DO PRODUTO (TOOLTIP)
// -------------------------------------------------------------------------

/**
 * Alterna a visibilidade da descrição (tooltip) de um produto.
 * @param {HTMLElement} element O elemento clicado (o ícone de informação).
 */
function toggleDescription(element) {
    // Pega o elemento irmão que é a descrição (tooltip)
    const tooltip = element.nextElementSibling;

    // Alterna a classe 'visible' para mostrar/esconder a tooltip clicada
    tooltip.classList.toggle('visible');

    // Fecha outras tooltips abertas para evitar múltiplas abertas ao mesmo tempo
    const allTooltips = document.querySelectorAll('.descricao-tooltip');
    allTooltips.forEach(item => {
        if (item !== tooltip) { // Se não for a tooltip que acabou de ser clicada
            item.classList.remove('visible');
        }
    });
}

// Fecha tooltips abertas ao clicar em qualquer lugar fora de um card de produto.
document.addEventListener('click', function(event) {
    const isClickInsideCard = event.target.closest('.card'); // Verifica se o clique foi dentro de um card

    if (!isClickInsideCard) {
        // Se o clique não foi dentro de nenhum card, fechar todas as tooltips
        document.querySelectorAll('.descricao-tooltip').forEach(tooltip => {
